...
def main(argv):
    ...
    for recording in recordings:
        ...
        # which will only be added as an eval token to all similar pronunciations
        # and get pronunciations for current recording.orth
        pronunciationEvaltokens=[]
        pronunciationPhons=[]
        pronunciationStates=[]
        ...
        for pronrecording in recordings:
            ...
            if pronrecording.classid in recording.evaltokens:            # i.e. we have a similar pronunced class for current L1O
                if pronrecording.recordid != recording.recordid:
                    wordLex.add(pronrecording.orth, [pronrecording.orth], [pronrecording.states], [pronrecording.orth.count('+')],
                                ...
                                evaltokens=[pronrecording.orth, recording.orth])
                else:
                    # just add it as an empty lemma
                    wordLex.add(recording.orth, [], [], [0],
                                addCoarticulation=False, checkPhoneme=False, checkLemma=True)
            else:
                wordLex.add(pronrecording.orth, [pronrecording.orth], [pronrecording.states], [pronrecording.orth.count('+')],
                            addCoarticulation=False, checkPhoneme=False, checkLemma=True)
